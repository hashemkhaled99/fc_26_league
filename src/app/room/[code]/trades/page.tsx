...
export default function TradesPage() {
  ...
  const load = useCallback(async () => {
    const res = await fetch(`/api/rooms/${code}/trades`);
  ...
  useEffect(() => {
    ...
    fetch(`/api/rooms/${code}/users/${partnerId}/squad`)
      .then((r) => r.json())
  ...
  async function submitTrade() {
    ...
    try {
      ...
      if (counterTradeId) {
        const res = await fetch(`/api/rooms/${code}/trades/${counterTradeId}`, {
          method: "POST",
  ...
  async function act(tradeId: string, action: "accept" | "reject") {
    ...
    setError("");
    try {
      const res = await fetch(`/api/rooms/${code}/trades/${tradeId}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
